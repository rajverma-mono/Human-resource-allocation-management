...
export class ProjectDetailsComponent implements OnInit {
  ...
  // Make these methods public so they can be called from template
...
onAction(action: string) {
  ...
  switch (action) {
    ...
    case 'export':
      ...
      break;
    case 'addEmployee':
      this.addEmployeeToProject(); // Will now go to allocation page
      break;
    case 'viewTeam':
      this.viewProjectTeam(); // Will show allocations or go to allocation page
      break;
    case 'matchResources':
      this.matchResources();
      break;
...
// Change from private to public since it's called from template
    ...
    html: `
      <div class="text-left">
        ...
        <div class="mb-4 p-3 bg-blue-50 border border-blue-100 rounded">
          <h5 class="font-medium text-blue-800 mb-2">What would you like to do?</h5>
          <div class="space-y-2">
        ...
        <p class="text-sm text-gray-600">
          You'll be redirected to the allocation page where you can manage all resource allocations.
        </p>
      </div>
    `,
    icon: 'question',
    showCancelButton: true,
    confirmButtonText: 'Go to Allocation Page',
...
private printAllocations() {
 ...
 // Change from private to public since they're called from template
addEmployeeToProject() {
  console.log('Add employee to project');
  this.handleResourceAllocation(); // Redirect to allocation page
}
...
viewProjectTeam() {
  ...
  if (this.allocations.length === 0) {
    ...
    // Show the detailed allocations modal
